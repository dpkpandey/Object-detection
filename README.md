...
```bash
git clone https://github.com/dpkpandey/Objectdetect
cd Objectdetect
```
...
3. Download the dataset and organize files:
   ...
   - Place annotation `.txt` files in the `labels` folder inside your `Objectdetect` directory.

## 7. Create YAML Configuration File
Create `Objectdetect.yaml` to define the dataset path and classes:
```yaml
path: C:\Users\YourComputer\Desktop\Objectdetect  #This is path of your current directory you can do pwd to see location and can copy that
train: images
val: images
names:
  0: #bottle
...
# Train the model
results = model.train(data="Objectdetect.yaml", batch=8, epochs=500)
```
...
This approximation works fine for Barramundi Object for any size becasue of emprical results on large number of Object. This approximation to pull length from detection is only 
...
Now I have uplodaded MainObjectcount.py file as well which is robust and more powerful interms of Object counting. You can change your last1.engine file according to output of your optimized or still you can use bestYm500.pt model as well. Choice is yours. For this if you use https://youtu.be/ppXhthoGPRkg link it will work better because of high FPS. 
## References
