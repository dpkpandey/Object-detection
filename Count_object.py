...
object_size = input("Size of object (in cm): ")
...
class ObjectCounter:
    def __init__(self, model_path, video_path, output_path, output_path1):
        ...
        '''IF we work on the max_age, higher number is better to sort and remember the tracking ID and number for longer period
           ...
           for 10 frames.
           Similarly, for min_hits =3 means that when object appeared for 3 consecutive frames then only it will assign new ID no. This is important to tune,
           because if object flip or change shape then camera may count twice near to counting line. This prevents double counting and by skipping for three consecutive frames, and 
    ...
    def process_frame(self, frame):
        ...
        for info in results:
            ...
            for box in boxes:
                ...
                # Filter detections with confidence above your desirable probability.
                if conf > 0.30: # If we have a bunch of objects coming at the same time, reduce confidence to detect more objects.
           ...
           # if self.line[0] < cx < self.line[2] and abs(cy - self.line[1]) <= 400:
            if self.line[0] <cx< self.line[2] and  (cy- 120) <self.line[1] <  (cy + 50):    
                ...
                if id not in self.counter_up:
                    ...
                    length_cm, weight_gm = self.calculate_object_properties(x1, y1, x2, y2)
                    self.total_length += length_cm
                    self.total_weight += weight_gm
                    self.object_count += 1
                    self.object_data.append((length_cm, weight_gm))

                    # Display length and weight only when object crosses the line
        ...
        # Display the total count of object
        cvzone.putTextRect(frame, f'Detected object No = {len(self.counter_up)}', [80, 34], colorR=(0, 0, 255), thickness=4, scale=2.3, border=3)
        #cvzone.putTextRect(frame, f'object = {len(self.counter_up)}', [10, 10], colorR=(0, 0, 255), thickness=1, scale=1, border=1)
    ...
    def calculate_object_properties(self, x1, y1, x2, y2):
        dpk = 0.118 / 2.51
        p = (x2 - x1) * 0.1089
        k = (y2 - y1) * 0.1089
        l_theta = max(k, p)
    ...
    def run(self):
        ...
        self.add_data_to_excel(self.object_count, source_from, source_to, object_size, average_length, average_weight)
        self.save_object_data_to_excel()
    ...
    def add_data_to_excel(self, object_count, source_from, source_to, object_size, average_length, average_weight):
        ...
        if os.path.exists(filename):
            ...
            ws['C1'] = "Source To"
            ws['D1'] = "Object Size"
            ws['E1'] = "Total Object Count"
            ws['F1'] = "Average Length of Object (cm)"
            ws['G1'] = "Average Weight of Object (gm)"
    ...
    def save_object_data_to_excel(self):
        ...
        if os.path.exists(filename):
            ...
            wb = Workbook()
            ws = wb.active
            ws.title = "Object Length and Weight"
            ws['A1'] = "Date"
            ws['B1'] = "Weight (gm)"
            ws['C1'] = "Length (cm)"
        ...
        for length, weight in self.object_data:
            ws[f'A{next_row}'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
if __name__ == "__main__":
    ...
    object_counter = ObjectCounter("lastY11m500.pt", "rds.mp4", output_file, output_file1)#bestyolo11m2000.pt
    object_counter.run()
